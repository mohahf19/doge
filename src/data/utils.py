...
def get_dataset(args, dataset=None) -> Dict[str, np.ndarray]:
    ...
    from slimpajama import SUBSET2META
    from slimpajama import get_slim_redpajama_6b, get_slim_redpajama
    ...
    if "wiki40b" in trg_dataset:
        ...
        for subset in subset_list:
            ...
            print(
                f"Subset {subset}: train[{len(subset_data['train'])}]|val[{subset_data['val']}]"
            )
        return rst_dict
    ...
    if "slim_6b" in trg_dataset:
        ...
        if subset == "all":
            ...
            for k in SLIMPAJAMA_DOMAINS:
                ...
                rst_dict["train"][k] = subset_data["train"]
                rst_dict["val"][k] = subset_data["val"][
                    : n_items_val * args.max_token_length
                ]
            return rst_dict
        elif subset == "mix":
            rst_dict = {}
            ...
            for k in SLIMPAJAMA_DOMAINS:
                subset_data = get_slim_redpajama(subset=k, num_proc=10)
                rst_dict["train"][k] = subset_data["train"][
                    : -n_items_mix_train * args.max_token_length
                ]
                rst_dict["val"][k] = subset_data["val"][
                    : n_items_mix_val * args.max_token_length
                ]
                mix_data_train.append(
                    subset_data["train"][-n_items_mix_train * args.max_token_length :]
                )
                mix_data_val.append(
                    subset_data["val"][-n_items_mix_val * args.max_token_length :]
                )
            ...
            np.random.shuffle(A)
            mix_train_data = np.concatenate(
                [mix_train_data[i : i + args.max_token_length] for i in A]
            )

            B = np.arange(0, len(mix_val_data), args.max_token_length)
            np.random.shuffle(B)
            mix_val_data = np.concatenate(
                [mix_val_data[i : i + args.max_token_length] for i in B]
            )
...
def get_data_folder() -> Path:
    if "MLOSCRATCH_DIR" in os.environ:
        return Path(os.environ["MLOSCRATCH_DIR"]) / "datasets"
    else:
        return Path(os.path.dirname(__file__)) / "datasets"
