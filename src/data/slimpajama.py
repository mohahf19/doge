...
def get_slimpajama(
    ...
    # }
    SLIM_DATA_PATH = str((get_data_folder() / "slim_redpajama").absolute())
    ...
    if not os.path.exists(os.path.join(SUBSET_PATH, "val.bin")):
        ...
        data_dict = {}
        data_dict["train"] = dataset[0].filter(
            lambda example: example["meta"]["redpajama_set_name"] == subset_name
        )
        data_dict["val"] = dataset[1].filter(
            lambda example: example["meta"]["redpajama_set_name"] == subset_name
        )
    ...
    train_data = np.memmap(
        os.path.join(SUBSET_PATH, "train.bin"), dtype=np.uint16, mode="r"
    )
    val_data = np.memmap(
        os.path.join(SUBSET_PATH, "val.bin"), dtype=np.uint16, mode="r"
    )
...
def get_slimpajama_6b(subset="arxiv", num_proc=40, return_torch=False):
    """Full: https://huggingface.co/datasets/cerebras/SlimPajama-627B
    6B-subset: DKYoon/SlimPajama-6B
    """
    # {
    ...
    #     "red_pajama_subset": "common_crawl" | "c4" | "github" | "books" | "arxiv" | "wikipedia" | "stackexchange"
    # }
    REDPAJIMA_DATA_PATH = str((get_data_folder() / "slimpajama-6b").absolute())
...
# get_slimpajama(subset="arxiv", num_proc=9, return_torch=True)
get_slimpajama_6b(subset="arxiv", num_proc=9, return_torch=True)
